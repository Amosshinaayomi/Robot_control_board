...
void motionControlTask(void *pvParameters) {
    ...
    for (;;) {
       if (!systemRunning) {
        ...
        float yawRate_rad = localData.yawRate * DEG_TO_RAD;   // if yawRate is in deg/s
        Serial.printf("yawRate is %.2f\n", yawRate_rad);
        ...
        motionController.update(leftTicksAvg, rightTicksAvg, localData.yaw, yawRate_rad, dt);
...
bool initAllHardware(uint8_t* errorCode)
{
    ...
    if(!encoderManager.begin(10000, TIM2)) {
